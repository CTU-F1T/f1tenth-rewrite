 ...
 *
 * Known issues:
 *    - When the angle between orientation of the vehicle and the reference
 *      trajectory is more than pi/2 the problem becomes infeasible.
 *
 * */
...
class MPC_node : public rclcpp::Node {
  ...
  int prediction_horizon = 55;  // MPC prediction horizon
  double dt = 0.020;  // time step used for model integration in MPC
  ...
  std::vector <Eigen::Matrix<double, NX, 1>> states_plan_copy;
...
public:
  ...
  MPC_node() : Node("mcp") {
    ...
    this->declare_parameter("tracking_position_cost", 15.0);
    ...
    callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&MPC_node::parametersCallback, this, std::placeholders::_1));

    // Load and parse reference
    std::string fname = "/home/nvidia/f1tenth/src/perception/recognition/particle_filter/maps/garaz.csv";  // Here change reference file TODO -> get reference from a topic
    int skip_lines = 1; //1 - ]]]
    ...
    if (file.is_open()) {
      while (getline(file, line)) {
        ...
        while (getline(str, word, ',')) { // , - ]]]
          ...
          if (col_ == 5) row_matrix[col_] *= 1.0; // <-- if want scale reference trajectory
          col++;
          col_++;
        }
...
//        this->mpc_planner.R.diagonal() << 0.01, 0.5;
    ...
    states_plan = std::vector < Eigen::Matrix < double, NX, 1
            >> (prediction_horizon, Eigen::Matrix<double, NX, 1>::Zero());
    states_plan_copy = std::vector < Eigen::Matrix < double, NX, 1
            >> (prediction_horizon, Eigen::Matrix<double, NX, 1>::Zero());
    inputs_plan = std::vector < Eigen::Matrix < double, NU, 1
            >> (prediction_horizon - 1, Eigen::Matrix<double, NU, 1>::Zero());
    inputs_plan_copy = std::vector < Eigen::Matrix < double, NU, 1
            >> (prediction_horizon - 1, Eigen::Matrix<double, NU, 1>::Zero());
    predicted_traj = std::vector < Eigen::Matrix < double, NX, 1
            >> (prediction_horizon, Eigen::Matrix<double, NX, 1>::Zero());
    reference = std::vector < Eigen::Matrix < double, NX, 1
            >> (prediction_horizon, Eigen::Matrix<double, NX, 1>::Zero());
...
//        std::cout << mpc_planner.state_matrix_sequence[0] << std::endl;
    ...
    pub_vis_whole_trajectory = this->create_publisher<visualization_msgs::msg::MarkerArray>("/vis/raceline",
                                                                                            qos_settings);
    ...
    pub_vis_global_reference = this->create_publisher<nav_msgs::msg::Path>("/global_reference", 1);
    pub_vis_mpc_optim_out = this->create_publisher<nav_msgs::msg::Path>("/mpc_optim_out", 1);
    pub_vis_mpc_optim_reference = this->create_publisher<nav_msgs::msg::Path>("/mpc_optim_reference", 1);
    ...
    timer_ = this->create_wall_timer(20ms, std::bind(&MPC_node::control_pub_timer, this));
...
//        std::cout << "Trajectory size:   " << this->mpc_planner.reference_trajectory.size() << std::endl;
    ...
    nav_msgs::msg::Path global_reference_;
    global_reference_.header.frame_id = "/map";
    global_reference_.header.stamp = this->get_clock()->now();
    ...
    for (int i = 0; i < this->mpc_planner.reference_trajectory.size(); i++) {
      ...
      global_reference_.poses.push_back(pp);
    }
    pub_vis_whole_trajectory->publish(array_m);
    pub_vis_global_reference->publish(global_reference_);
  }
...
//      this->control_lock.unlock();
  ...
  void control_pub_timer() {
    ...
    float steer_command = 0.0;

    if (this->iter_since_last_succ > 20){
...
//      this->drive_steering_angle = 0.0;
      ...
      this->drive_steering_angle += this->inputs_plan_copy[0][1] * 0.02;

      // TODO: Add this check to the model lib
      if (this->drive_speed > this->mpc_planner.model.max_speed - 0.1){
        this->drive_speed = this->mpc_planner.model.max_speed - 0.1;
      }

      if (this->drive_speed < this->mpc_planner.model.min_speed + 0.1){
        this->drive_speed = this->mpc_planner.model.min_speed + 0.1;
      }

      if (this->drive_steering_angle > this->mpc_planner.model.max_steer_angle - 0.1){
        this->drive_steering_angle = this->mpc_planner.model.max_steer_angle - 0.1;
      }

      if (this->drive_steering_angle < this->mpc_planner.model.min_steer_angle + 0.1){
        this->drive_steering_angle = this->mpc_planner.model.min_steer_angle + 0.1;
      }


//      this->drive_speed = this->states_plan_copy[1][3];
//      this->drive_steering_angle = this->states_plan_copy[1][4];
      this->iter_since_last_succ += 1;

      vel_command = this->drive_speed;
      steer_command = this->drive_steering_angle;
    ...
    command_param_steer.value = steer_command;// + 0.1 * this->mpc_planner.patch_ref_trajectory_info[0][0]; // steering angle
    ...
    this->control_lock.lock();
    command_param_drive.value = vel_command;
    this->control_lock.unlock();
...
//        std::cout << "Pos_update_time  " << pos_update_time.count() << std::endl;
    ...
    this->control_lock.lock();
    this->x0
            << msg->pose.pose.position.x, msg->pose.pose.position.y, yaw, this->drive_speed, this->drive_steering_angle;
    this->control_lock.unlock();


    // lidar to center of the rear axle
    this->x0(0) = this->x0(0) - cos(yaw) * 0.15;
    this->x0(1) = this->x0(1) - sin(yaw) * 0.15;
...
//    std::cout << "Start loop: \n" << std::endl;
    ...
    int plan_error = 0;
    plan_error = this->mpc_planner.plan(this->states_plan, this->inputs_plan, this->predicted_traj, this->reference,
                                        this->x0);
    ...
    for (int i = 0; i < this->inputs_plan.size(); i++) {
      if (this->inputs_plan[i][0] > this->mpc_planner.model.max_acc + 0.1 ||
          this->inputs_plan[i][0] < this->mpc_planner.model.max_dcc - 0.1 ||
          this->inputs_plan[i][1] > this->mpc_planner.model.max_steer_vel + 0.1 ||
          this->inputs_plan[i][1] < -this->mpc_planner.model.max_steer_vel - 0.1) {
        std::cout << "Solver error: Solution out of bounds  " << i << "  " << this->inputs_plan[i][0] << "  "
                  << this->inputs_plan[i][1] << std::endl;
        return;
      }
    }

//    std::cout << this->inputs_plan[0][0] << "   " << this->inputs_plan[0][1] << "\n" << std::endl;

    // Uncomment this for debug topics publishing // TODO DEBUG parameter on the input
    /* 
    this->control_lock.lock();
    this->inputs_plan_copy = this->inputs_plan;
    this->states_plan_copy = this->states_plan;
    this->iter_since_last_succ = 0;
    this->control_lock.unlock();
    ...
    tracking_error.data = sqrt(
            pow(this->reference[0][0] - this->x0[0], 2.0) + pow(this->reference[0][1] - this->x0[1], 2.0));
    ...
    vis_states(this->states_plan, this->pub_vis_mpc_optim_out);
    vis_states(this->reference, this->pub_vis_mpc_optim_reference);
*/



// Simple logging to a file - temporary solution
...
//        }
  ...
  // Uncomment this for printing runtimes of different parts of the code
  // Currently biggest slowdown is "Cast" function that copies matrices A, B, C and reference to the sparse matrices for the OSQP (This is even slower than solve time)
  /*
        std::cout << "Plan runtime: " << plan_time.count() / 1000.0 << " [ms], " <<
                  "Predict motion :  " << this->mpc_planner.predict_motion_time.count() / 1000.0 << " [ms], " <<
                  "Linearization :  " << this->mpc_planner.linearization_time.count() / 1000.0 << " [ms], " <<
                  "Ref calc :  " << this->mpc_planner.ref_time.count() / 1000.0 << " [ms], " <<
                  "Cast :  " << this->mpc_planner.cast_time.count() / 1000.0 << " [ms], " <<
                  "Solver :  " << this->mpc_planner.solver_time.count() / 1000.0 << " [ms], " <<
                  "Time between loops:  " << pos_update_time.count() / 1000.0 << " [ms]" << std::endl;
  */
  }

  void vis_states(std::vector <Eigen::Matrix<double, NX, 1>> &states,
    ...
    for (int i = 0; i < states.size(); i++) {
      geometry_msgs::msg::PoseStamped pp;
      pp.pose.position.x = states[i][0];
      pp.pose.position.y = states[i][1];
      pp.header.frame_id = "/map";
      pp.header.stamp = this->get_clock()->now();
  ...
  void visualize_position_from_states(std::vector <Eigen::Matrix<double, NX, 1>> &states,
                                      ...
                                      double scale) {
    // visualize_position_from_states(this->predicted_traj, this->pub_vis_predicted_trajectory, 0, 0.5, 0, 0.1);
    visualization_msgs::msg::MarkerArray array_m;
    for (int i = 0; i < states.size(); i++) {
      rclcpp::Time now = this->get_clock()->now();
      visualization_msgs::msg::Marker mark;
  ...
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_vis_global_reference;
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_vis_mpc_optim_out;
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_vis_mpc_optim_reference;
